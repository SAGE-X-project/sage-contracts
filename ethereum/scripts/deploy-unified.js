  ...
  try {
    ...
    if (config.testAgents) {
      ...
      for (let i = 0; i < agents.length; i++) {
        ...
        if (isLocal) {
          ...
          const messageHash = hre.ethers.keccak256(
            hre.ethers.solidityPacked(
              ...
              ["TEST_MODE:", signer.address, keyHash]
            )
          );
        ...
        // Register agent
        tx = await sageRegistry.connect(signer).registerAgent(
          agent.did,
          agent.name,
          agent.description,
          agent.endpoint,
    ...
    fs.writeFileSync(latestFile, JSON.stringify(deploymentInfo, (key, value) => {
      if (typeof value === 'bigint') {
        return value.toString();
      }
      return value;
...
`;
    ...
    if (Object.keys(deploymentInfo.contracts).length > 0) {
      ...
      fs.writeFileSync(failedFile, JSON.stringify(deploymentInfo, (key, value) => {
        if (typeof value === 'bigint') {
          return value.toString();
        }
        return value;
