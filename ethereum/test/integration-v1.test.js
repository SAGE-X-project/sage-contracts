...
describe("SAGE Registry Integration Tests", function () {
  ...
  beforeEach(async function () {
    ...
    await registry.setBeforeRegisterHook(await verificationHook.getAddress());
    await registry.setAfterRegisterHook(await verificationHook.getAddress());
  });
  ...
  describe("Agent Registration Flow", function () {
    it("should successfully register an agent with valid signature", async function () {
      ...
      const logs = await registry.queryFilter(registry.filters.AgentRegistered(), receipt.blockNumber, receipt.blockNumber);
      const event = logs[0];
    ...
    it("should reject registration with invalid signature", async function () {
      ...
      const signature = await user1.signMessage(ethers.getBytes(wrongMessage));
    ...
    it("should enforce rate limiting through hook", async function () {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        // Wait for cooldown period between registrations
        await ethers.provider.send("evm_increaseTime", [61]); // 1 minute + 1 second
        await ethers.provider.send("evm_mine");
      }

      // 6th registration should fail due to daily limit
      const spamData = {
        did: "did:sage:spam6",
        name: "Spam Agent 6",
