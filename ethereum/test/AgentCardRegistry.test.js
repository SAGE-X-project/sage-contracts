...
describe("AgentCardRegistry", function () {
  ...
  function createRegistrationParams(
    did,
    name,
    keys,
    keyTypes,
  ...
  async function commitAndAdvanceTime(user, did, keys, salt) {
    ...
    await registry
      .connect(user)
      .commitRegistration(commitHash, { value: STAKE });
  ...
  describe("V3.1: Commit-Reveal Pattern", function () {
    it("R3.1.1: Should require sufficient stake for commitment", async function () {
      ...
      await expect(
        registry
          .connect(user1)
          .commitRegistration(commitHash, { value: parseEther("0.005") })
    ...
    it("R3.1.8: Should prevent front-running attack", async function () {
      ...
      await registry
        .connect(attacker)
        .commitRegistration(attackerCommitHash, { value: STAKE });
  ...
  describe("V3.3: Key Management", function () {
    ...
    it("R3.3.2: Should reject addKey if max keys reached", async function () {
      ...
      for (let i = 0; i < 9; i++) {
        ...
        await registry.connect(user1).addKey(agentId, wallet.publicKey, 0, sig);
      }
      ...
      await expect(
        registry.connect(user1).addKey(agentId, wallet11.publicKey, 0, sig11)
    ...
    it("R3.3.3: Should verify ownership when adding key", async function () {
      ...
      await expect(
        registry.connect(user1).addKey(agentId, validKey2, 0, wrongSig)
      ).to.be.revertedWith("Invalid ECDSA signature");
    });

    it("R3.3.4: Should only allow owner to add key", async function () {
      await expect(
        registry.connect(user2).addKey(agentId, validKey2, 0, validSig2)
    ...
    it("R3.3.5: Should remove key successfully", async function () {
      ...
      const sig = await createSignature(user1);
      await registry.connect(user1).addKey(agentId, validKey2, 0, sig);
    ...
    it("R3.3.9: Should emit key operation events", async function () {
      ...
      const sig = await createSignature(user1);
      await expect(
        registry.connect(user1).addKey(agentId, validKey2, 0, sig)
  ...
  describe("V3.4: Agent Management", function () {
    ...
    it("R3.4.1: Should update endpoint", async function () {
      ...
      await registry
        .connect(user1)
        .updateAgent(agentId, newEndpoint, '{"capabilities": []}');
    ...
    it("R3.4.2: Should update capabilities", async function () {
      ...
      await registry
        .connect(user1)
        .updateAgent(agentId, "https://example.com", newCapabilities);
    ...
    it("R3.4.3: Should increment nonce on update", async function () {
      ...
      await registry
        .connect(user1)
        .updateAgent(agentId, "https://new.com", '{"capabilities": []}');
    ...
    it("R3.4.4: Should only allow owner to update", async function () {
      await expect(
        registry
          .connect(user2)
          .updateAgent(agentId, "https://hacker.com", '{"capabilities": []}')
      ).to.be.revertedWith("Not agent owner");
    });
    ...
    it("R3.4.5: Should activate agent after time-lock", async function () {
      ...
      await expect(registry.activateAgent(agentId)).to.be.revertedWith(
  ...
  describe("V3.5: Security Features", function () {
    ...
    it("R3.5.2: Should allow emergency pause", async function () {
      await expect(registry.connect(owner).pause()).to.not.be.revert(ethers);
    ...
    it("R3.5.5: Should only allow owner to pause", async function () {
      await expect(registry.connect(user1).pause()).to.be.revert(ethers); // Ownable: caller is not the owner
    });

    it("R3.5.6: Should only allow owner to unpause", async function () {
      await registry.connect(owner).pause();

      await expect(registry.connect(user1).unpause()).to.be.revert(ethers); // Ownable: caller is not the owner

      // Owner can unpause
      await expect(registry.connect(owner).unpause()).to.not.be.revert(ethers);
    });
    ...
    it("R3.5.8: Should prevent cross-chain replay (chainId)", async function () {
      ...
      await registry
        .connect(user1)
        .commitRegistration(commitHash1, { value: STAKE });
  ...
  describe("V3.6: KME Key Management", function () {
    ...
    describe("3.6.1: KME Key Registration", function () {
      it("R3.6.1: Should store X25519 key in kemPublicKey field during registration", async function () {
        const salt = ethers.randomBytes(32);
        ...
        const tx = await registry
          .connect(user1)
          .registerAgentWithParams(params);
        const receipt = await tx.wait();
        ...
        // Verify kemPublicKey was stored
        const agent = await registry.getAgent(agentId);
        expect(agent.kemPublicKey).to.equal(ethers.hexlify(x25519Key));

        // Verify getKEMKey() returns correct key
        const KEMKey = await registry.getKEMKey(agentId);
        expect(KEMKey).to.equal(ethers.hexlify(x25519Key));
      });

      it("R3.6.2: Should allow registration without X25519 key (empty kemPublicKey)", async function () {
        const salt = ethers.randomBytes(32);
        ...
        // Verify kemPublicKey is empty
        const agent = await registry.getAgent(agentId);
        expect(agent.kemPublicKey).to.equal("0x");

        // getKEMKey() should return empty bytes
        const KEMKey = await registry.getKEMKey(agentId);
        expect(KEMKey).to.equal("0x");
      });
      ...
      it("R3.6.5: Should emit KEMKeyUpdated event on registration with X25519", async function () {
        const salt = ethers.randomBytes(32);
        const x25519Key = ethers.randomBytes(32);
        ...
        // Find KEMKeyUpdated event
        const kmeEvent = receipt.logs.find(
          (log) => log.fragment && log.fragment.name === "KEMKeyUpdated"
        );
    ...
    describe("3.6.2: X25519 Ownership Verification", function () {
      it("R3.6.6: Should reject X25519 registration without signature", async function () {
        ...
        await expect(
          ...
          "X25519 requires ECDSA signature for ownership proof"
        );
      });
      ...
      it("R3.6.8: Should prevent attacker from registering others' X25519 key", async function () {
        ...
        const attackerX25519Sig = await createX25519Signature(
          attacker,
          victimX25519Key
        );
    ...
    describe("3.6.3: KME Key Retrieval", function () {
      ...
      beforeEach(async function () {
        ...
        const tx1 = await registry
          .connect(user1)
          .registerAgentWithParams(params1);
        ...
        const tx2 = await registry
          .connect(user2)
          .registerAgentWithParams(params2);
      ...
      it("R3.6.9: Should retrieve kemPublicKey via getKEMKey()", async function () {
        const KEMKey = await registry.getKEMKey(agentIdWithKME);
        expect(KEMKey).to.equal(ethers.hexlify(x25519TestKey));
      });

      it("R3.6.10: Should return empty bytes for agent without X25519 key", async function () {
        const KEMKey = await registry.getKEMKey(agentIdWithoutKME);
        expect(KEMKey).to.equal("0x");
      });

      it("R3.6.11: Should return kemPublicKey in getAgent() response", async function () {
        const agent = await registry.getAgent(agentIdWithKME);
        expect(agent.kemPublicKey).to.equal(ethers.hexlify(x25519TestKey));
      });

      it("R3.6.12: Should revert getKEMKey() for non-existent agent", async function () {
        const fakeAgentId = ethers.randomBytes(32);
        await expect(registry.getKEMKey(fakeAgentId)).to.be.revertedWith(
          "Agent not found"
        );
      });
    });
    ...
    describe("3.6.4: KME Key Updates", function () {
      ...
      it("R3.6.13: Should update kemPublicKey via updateKEMKey()", async function () {
        const newX25519Key = ethers.randomBytes(32);
        const newSig = await createX25519Signature(user1, newX25519Key);

        await registry
          .connect(user1)
          .updateKEMKey(agentId, newX25519Key, newSig);

        const KEMKey = await registry.getKEMKey(agentId);
        expect(KEMKey).to.equal(ethers.hexlify(newX25519Key));
      });

      it("R3.6.14: Should emit KEMKeyUpdated event on successful update", async function () {
        const newX25519Key = ethers.randomBytes(32);
        const newSig = await createX25519Signature(user1, newX25519Key);

        await expect(
          registry.connect(user1).updateKEMKey(agentId, newX25519Key, newSig)
        )
          .to.emit(registry, "KEMKeyUpdated")
          .withArgs(
            agentId,
            keccak256(newX25519Key),
      ...
      it("R3.6.15: Should prevent unauthorized KME key updates", async function () {
        ...
        await expect(
          registry.connect(user2).updateKEMKey(agentId, newX25519Key, newSig)
        ).to.be.revertedWith("Not agent owner");
      });
    });
  });
