...
describe("AgentCardStorage", function () {
  ...
  before(async function () {
    ...
    const StorageTestFactory = await ethers.getContractFactory(
      "AgentCardStorageTest"
    );
  ...
  describe("V1.1: Struct Definitions", function () {
    ...
    it("S1.1.1: Should create AgentMetadata with all 12 fields", async function () {
      ...
      const metadata = {
        ...
        active: false,
        chainId: 31337,
        kemPublicKey: "0x", // Empty KME key (optional field)
      };
      ...
      expect(retrieved.kemPublicKey).to.equal(metadata.kemPublicKey); // ✅ Verify KME key
    });
    ...
    it("S1.1.5: Should have correct field types for all structs", async function () {
      ...
      await storage.setAgentMetadata(agentId, {
        ...
        active: true, // bool
        chainId: 1, // uint256
        kemPublicKey: "0x", // bytes (optional)
      });
  ...
  describe("V1.4: Events", function () {
    ...
    it("S1.4.1: Should emit AgentRegistered with correct params", async function () {
      ...
      await expect(
        storage.emitAgentRegistered(agentId, did, user1.address, timestamp)
      )
    ...
    it("S1.4.6: Should emit CommitmentRecorded with correct params", async function () {
      ...
      await expect(
        storage.emitCommitmentRecorded(user1.address, commitHash, timestamp)
      )
